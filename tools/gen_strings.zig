...
//! frontend the build itself uses. A separate generated validation module
//! checks them against `@import("ipmi_c")` in the existing test and selected
//! module graph; the table data itself does not import the C bridge.
//!
...
//!     zig run tools/gen_strings.zig -- [--check] \
//!         lib/ipmi_strings.c ipmi_c.zig src/zig/util/strings_tables.zig
//!     zig fmt src/zig/util/strings_tables{,_validation}.zig
...
pub fn main(init: std.process.Init) !void {
    ...
    if (!(args.len == 4 or check)) {
        fatal("usage: gen_strings [--check] <ipmi_strings.c> <translated ipmi_c.zig> <output.zig>", .{});
    }
    const paths = args[1 + @as(usize, @intFromBool(check)) ..];
    const output = paths[2];
    if (!std.mem.endsWith(u8, output, ".zig")) fatal("expected a .zig output path: {s}", .{output});
    const validation_path = try std.fmt.allocPrint(gpa, "{s}_validation.zig", .{output[0 .. output.len - 4]});

    const source = try readFile(paths[0]);
    const translated = try readFile(paths[1]);
    ...
    try emit(&out, &validation, tables.items, constants.items);
    if (check) {
        try checkGenerated(output, out.items);
        try checkGenerated(validation_path, validation.items);
    } else {
        try std.Io.Dir.cwd().writeFile(io, .{ .sub_path = output, .data = out.items });
        try std.Io.Dir.cwd().writeFile(io, .{ .sub_path = validation_path, .data = validation.items });
    }
    ...
    try stdout.interface.print(
        "{s} and {s}: {d} tables, {d} entries, {d} resolved constants\n",
        .{ output, validation_path, tables.items.len, entries, constants.items.len },
    );
...
fn checkGenerated(path: []const u8, generated: []const u8) !void {
    const actual = try readFile(path);
    if (!std.mem.eql(u8, actual, generated)) {
        fatal("{s} is out of date; regenerate it", .{path});
    }
}
...
const validation_preamble =
    ...
    \\//! Imported by `strings.zig`, not by the standalone table data.
    \\
...
fn emit(out: *std.ArrayList(u8), validation: *std.ArrayList(u8), tables: []const Table, constants: []const Constant) !void {
    ...
    try validation.appendSlice(gpa, validation_preamble);
    try validation.appendSlice(gpa, "\ncomptime {\n");
    try validation.appendSlice(gpa,
    ...
    try validation.append(gpa, '\n');
    for (tables) |table| {
        if (table.is_static) continue;
        try validation.print(
            gpa,
    ...
    try validation.append(gpa, '\n');
    for (constants) |constant| {
        try validation.print(gpa, "    assertConst(tables.{s}, c.{s});\n", .{ constant.zig_name, constant.c_name });
    }
    try validation.appendSlice(gpa, "}\n");

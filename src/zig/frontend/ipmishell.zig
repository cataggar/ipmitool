//! Optional interactive shell, script executor and runtime settings.
//! Replaces src/ipmishell.c without linking readline when selected.
...
fn parse(arena: std.mem.Allocator, text: []const u8, comments: bool) !std.ArrayList([*:0]u8) {
...
fn execMain(intf: *Intf, argc: c_int, argv: [*c][*c]u8) callconv(.c) c_int {
    ...
    while (c.fgets(&buf, buf.len, fp) != null) {
        ...
        var arena_state = std.heap.ArenaAllocator.init(allocator);
        defer arena_state.deinit();
        const parsed = parse(arena_state.allocator(), buf[0..len], true) catch |err| {
            c.lprintf(log.Level.err, "Invalid command line: %s", @errorName(err).ptr);
            rc = -1;
            continue;
        };
        if (parsed.items.len == 0) continue;
        var args: [max_args + 1][*c]u8 = @splat(null);
